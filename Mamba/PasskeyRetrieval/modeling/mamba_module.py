...
def function_(input_tensor, scalars):
    ...
    scalar_indices[: remainder * (rows_per_scalar + 1)] = torch.arange(
        remainder
    ).repeat_interleave(rows_per_scalar + 1)

    # Handle the remaining scalars with the normal number of rows
    scalar_indices[remainder * (rows_per_scalar + 1) :] = torch.arange(
        remainder, D
    ).repeat_interleave(rows_per_scalar)
...
class Mamba(nn.Module):
    def __init__(
        ...
        self.mamba_scale = nn.Parameter(
            torch.from_numpy(numpy.ones((self.d_inner, self.d_state))).bfloat16().cuda(),
            requires_grad=True,
        )
    ...
    def forward(self, hidden_states, delta_ratio=None, inference_params=None):
        ...
        A = -torch.exp(
            self.A_log.float() * self.mamba_scale
        ) # (d_inner, d_state)
...
class Block(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.mixer(
            hidden_states, delta_ratio=delta_ratio, inference_params=inference_params
        )
