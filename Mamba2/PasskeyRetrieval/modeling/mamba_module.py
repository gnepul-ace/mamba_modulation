...
class Mamba2(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        self.mamba_scale = nn.Parameter(
            torch.from_numpy(numpy.random.rand(self.nheads)).bfloat16().cuda(),
            requires_grad=True
        )
    ...
    def forward(self, u, delta_ratio=None, seqlen=None, seq_idx=None, cu_seqlens=None, inference_params=None):
        """
        ...
        A = -torch.exp(self.A_log.float()) * self.mamba_scale.to(dtype=self.A_log.float().dtype)  # (nheads) or (d_inner, d_state)
        # print(self.A_log.shape)
        dt_limit_kwargs = {} if self.dt_limit == (0.0, float("inf")) else dict(dt_limit=self.dt_limit)
        if self.use_mem_eff_path and inference_params is None and not delta_ratio:
...
class Block(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.mixer(hidden_states, delta_ratio=delta_ratio, inference_params=inference_params, **mixer_kwargs)
